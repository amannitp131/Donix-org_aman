...
const OrganDonationTab = () => {
  ...
  const fetchPendingVerifications = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/getHospitalAppointments",
        {
  ...
  const fetchPendingRequests = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/getAllRequestedOrgan",
        {
          headers: { authorization: `Bearer ${token}` },
        }
      );
  ...
  const approveAppointment = async (appointmentId) => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/approveAppointment/${appointmentId}`,
        {},
        { headers: { authorization: `Bearer ${token}` } }
      );
  ...
  const approveRequest = async (requestId) => {
    try {
      ...
      const response = await axios.put(
        `http://localhost:5000/approveRequest/${requestId}`,
        {},
        { headers: { authorization: `Bearer ${token}` } }
      );
