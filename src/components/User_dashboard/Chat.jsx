...
function ChatApp() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:10000/api/users/get-all")
  ...
  const handleUserClick = async (userId) => {
    ...
    try {
      const response = await axios.get("http://localhost:10000/api/users/chat-history", {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://localhost:10000/api/users/message-send", newMessage);
