...
const Find = () => {
  ...
  useEffect(() => {
    const fetchOrgans = async () => {
      try {
        const response = await fetch("http://localhost:10000/fetchOrgan");
        if (!response.ok) {
  ...
  const toggleDropdown = (hospitalName: string) => {
  ...
  const handleRequest = async () => {
    try {
      ...
      const response = await fetch("http://localhost:10000/requestOrgan", {
        method: "POST",
        headers: {
