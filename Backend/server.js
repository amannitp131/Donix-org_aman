...
app.post('/send-otp', async (req, res) => {
    try {
        ...
        otpStore[email] = { otp, expiresAt: Date.now() + 600000 }; // Store OTP with a 5-minute expiration
...
app.post("/userRegistration", async (req, res) => {
  try {
    ...
    res.status(201).json({
      message: "New user added successfully. Please check your email for verification.",
      redirectUrl: "/verify",
    });
  } catch (error) {
...
app.post("/login", async (req, res) => {
    try {
      ...
      const token = jwt.sign(
        { id: user._id, email: user.email, role, fullName: user.fullName },
        secret_key,
        { expiresIn: "1h" }
      );
...
app.post('/send-otp', async (req, res) => {
  try {
    ...
    otpStore[email] = { otp, expiresAt: Date.now() + 600000 }; // Store OTP with a 10-minute expiration
...
app.get("/admin/hospitals", async (req, res) => {
  try {
    ...
    // Fetch unverified hospitals using Mongoose
    ...
    // Respond with the list of unverified hospitals
  ...
  try {
    ...
    try {
      const result = await client.predict(
        0, // Replace `0` with the correct fn_index for the function
        [heart_rate, bp, oxygen_saturation, symptoms, feedback],
        "/process_feedback"
      );

      // Return the response from the Gradio client
      return res.status(200).json({ response: result.data });
    } catch (error) {
      console.error("Error in AI prediction:", error.message);
      return res.status(500).json({ error: "AI response failed", details: error.message });
...
app.post("/predicts", async (req, res) => {
  try {
    ...
    try {
      // Call the Gradio model's predict function
